...
MukChoice는 단순히 주변 맛집을 검색하는 서비스를 만드는 것이 아니라,

'오늘 뭐 먹지?'라는 일상적인 의사결정을 즐거운 경험으로 만드는 것을 목표로 하였습니다.
...
JWT 없이도 안전한 인증 구조가 필요했습니다.

#### 해결

Spring Security OAuth2를 적용하여 카카오 로그인 기반 인증을 구현했습니다.

#### 결과

소셜 로그인만으로 간편한 인증 환경을 구축했습니다.

### 그룹 초대

#### 문제

초대 링크를 공유하면서도 권한을 안전하게 관리해야 했습니다.

#### 해결

초대 토큰을 생성하고 그룹 가입 시 검증하도록 구현했습니다.

### 위치 기반 검색

#### 문제

GPS 위치를 기반으로 사용자가 빠르게 맛집을 찾을 수 있어야 했습니다.

#### 해결

현재 위치 기준 검색 반경을 제한하여 필요한 데이터만 조회하도록 구현했습니다.
...
서비스 구조와 운영 환경을 함께 고려하는 것이 중요하다는 것을 배웠습니다.
...
를 직접 구현하면서 실제 서비스를 운영하는 경험을 할 수 있었습니다.
